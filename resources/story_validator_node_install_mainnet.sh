...
if [[ "$SETUP_UFW" =~ ^[Yy]$ ]]; then
    ...
    sudo ufw allow ${STORY_PORT}303/tcp comment "Story-geth Mainnet P2P"
    sudo ufw allow ${STORY_PORT}303/udp comment "Story-geth Mainnet discovery"
    sudo ufw allow ${STORY_PORT}656/tcp comment "Story Mainnet CometBFT P2P"
...
if [ "$VERSION_ID" = "22.04" ]; then
    ...
    mkdir -p story-geth
    cd story-geth
    wget -O v1.2.1.tar.gz https://github.com/piplabs/story-geth/archive/refs/tags/v1.2.1.tar.gz
    tar -xzf v1.2.1.tar.gz
    cd story-geth-1.2.1
    make geth
    cp $HOME/story-geth/story-geth-1.2.1/build/bin/geth $HOME/go/bin/
    ...
    cp story-geth-v1.2.1/geth-linux-amd64 $HOME/go/bin/geth
    sudo chown -R $USER:$USER $HOME/go/bin/geth
    sudo chmod +x $HOME/go/bin/geth
else
...
wget -O $HOME/story-v1.8.0/story https://github.com/piplabs/story/releases/download/v1.8.0/story-linux-amd64
...
[Service]
User=$USER
ExecStart=$(which geth) --story --syncmode full --http --http.api eth,net,web3,engine --http.vhosts '*' --http.addr 127.0.0.1 --http.port ${STORY_PORT}545 --ws --ws.api eth,web3,net,txpool --ws.addr 127.0.0.1 --ws.port ${STORY_PORT}546 --authrpc.port ${STORY_PORT}551 --port ${STORY_PORT}303 --discovery.port ${STORY_PORT}303
StandardOutput=journal
StandardError=journal
Restart=on-failure
RestartSec=3
