...
init_cosmovisor() {
    ...
    if ! wget -p $HOME/story-v1.8.0 https://github.com/piplabs/story/releases/download/v1.8.0/story-linux-amd64 -O $HOME/story-v1.8.0/story; then
        echo "Failed to download the genesis binary. Exiting."
        exit 1
    fi

    # Initialize cosmovisor
    if ! cosmovisor init $HOME/story-v1.8.0/story; then
...
echo -e "k. ${YELLOW}v1.8.0${RESET} (${GREEN}Seneca - Mandatory Mainnet Hardfork${RESET} Upgrade height: $(LC_NUMERIC='en_US.UTF-8' printf "%'d" $((realtime_block_height + 100))))"
...
case $choice in
    ...
    j)
        ...
        ;;
    k)
        update_version "v1.8.0" "https://github.com/piplabs/story/releases/download/v1.8.0" $((realtime_block_height + 100))
        ;;
    *)
