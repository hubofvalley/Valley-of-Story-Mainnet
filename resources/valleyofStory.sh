...
- Story Mainnet Guide on GitHub by Grand Valley: ${BLUE}https://github.com/hubofvalley/Mainnet-Guides/tree/main/Story${RESET}
...
echo "export STORY_CHAIN_ID=1514" >> ~/.bash_profile
echo "export STORY_NETWORK_NAME=story" >> ~/.bash_profile
...
function send_ip_token() {
    ...
    # using the user-provided RPC: https://mainnet.storyrpc.io
    # adding path to ensure cast is found if just installed
    export PATH="$HOME/.foundry/bin:$PATH"
    
    cast send --rpc-url https://mainnet.storyrpc.io --private-key "$PRIVATE_KEY" --value "${AMOUNT}ether" "$RECIPIENT"
...
function show_node_status() {
    ...
    execution_peers_raw=$(geth --exec "net.peerCount" attach $HOME/.story/geth/story/geth.ipc)
...
function install_story_app() {
    ...
    mkdir -p story-v1.8.0
    wget -O story-v1.8.0/story https://github.com/piplabs/story/releases/download/v1.8.0/story-linux-amd64
    cp story-v1.8.0/story $HOME/go/bin/story
...
function show_guidelines() {
    ...
    echo "   i. Schedule Stop/Restart Validator Node: Schedules stop/disable or restart/enable for consensus and geth services."
    echo "      - Guide: Use this option to set a time using atd."
    echo -e "${GREEN}Install Story App only: Installs the Story app (v1.8.0) for executing transactions without running the node.${RESET}"
...
function menu() {
    ...
    echo -e "${GREEN}4. Install the Story App (v1.8.0) only to execute transactions without running a node${RESET}"
